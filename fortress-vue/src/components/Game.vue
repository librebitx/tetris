  ...
  <div class="game-container">
    ...
    <div v-if="gameState.config?.winner && !isReviewing" class="overlay">
      <div class="overlay-card"
        :class="isSolo ? 'accent-red' : (gameState.config.winner === 'Red' ? 'accent-red' : (gameState.config.winner === 'Blue' ? 'accent-blue' : 'accent-draw'))">

        <!-- ── Banner ── -->
        <div class="result-banner"
          :class="isSolo ? 't-red' : (gameState.config.winner === 'Red' ? 't-red' : (gameState.config.winner === 'Blue' ? 't-blue' : 't-draw'))">
          <template v-if="isSolo">挑战结束</template>
          <template v-else>
            <span class="banner-mode">{{ gameState.config.mode === 'time' ? '限时' : '积分' }}</span>
        ...
        <div v-if="!isSolo" class="ready-row">
        ...
        <div class="action-group">
          <button v-if="isSolo" class="action-btn primary" @click="handleRestartClick">再来一局</button>
          <button v-else-if="isHost" class="action-btn primary" @click="handleRestartClick"
            :disabled="amIReady" :class="{ disabled: amIReady, 'pulse-cyan': isBlueReady && !amIReady }">
            {{ amIReady ? '已就绪 - 等待蓝方' : (isBlueReady ? '蓝方已就绪 - 再来一局' : '再来一局') }}
          </button>
          <button v-else class="action-btn primary" @click="handleRestartClick"
            :disabled="amIReady" :class="{ disabled: amIReady, 'pulse-cyan': isRedReady && !amIReady }">
            {{ amIReady ? '已就绪 - 等待红方' : (isRedReady ? '红方已就绪 - 再来一局' : '再来一局') }}
          </button>
          <button class="action-btn secondary" @click="toggleReview">对局复盘</button>
          <button class="action-btn danger" @click="handleLeave">退出房间</button>
        </div>
...
.overlay-card {
  ...
  border-top: 6px solid #555; /* overridden by accent classes */
  border-radius: 0;
  padding: 0 0 20px;
  text-align: center;
  width: 92%;
...
.overlay-card.accent-red  { border-top-color: #ff6b6b; }
.overlay-card.accent-blue { border-top-color: #4dabf7; }
.overlay-card.accent-draw { border-top-color: #888; }
...
/* Action Group — unified button stack */
.action-group {
  display: flex;
  flex-direction: column;
  gap: 8px;
...
/* h-main-row padding inside overlay-card */
.h-main-row {
